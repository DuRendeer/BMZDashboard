...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['message'])) {
    ...
    if (!empty($message) && !empty($token) && !empty($userId) && strlen($message) > 3) {
        // Passo 1: Criar canal de DM com o usuário
        ...
        $dmContext = stream_context_create([
            'http' => [
                ...
                'header' => [
                    ...
                    'User-Agent: DiscordBot (DicaAnonima, 1.0)'
                ],
                'content' => $dmData,
                'ignore_errors' => true
            ]
        ...
        if ($dmResponse !== false) {
            ...
            if (isset($dmChannel['id'])) {
                ...
                if ($messageResponse !== false) {
                    ...
                    // Debug temporário - vamos ver o erro
                    $errorInfo = error_get_last();
                    $httpCode = isset($http_response_header[0]) ? $http_response_header[0] : 'N/A';
                    header('Location: ' . $_SERVER['PHP_SELF'] . '?error=msg&debug=' . urlencode($httpCode));
                    exit;
                }
            } else {
                // Debug temporário - vamos ver a resposta
                header('Location: ' . $_SERVER['PHP_SELF'] . '?error=dm&debug=' . urlencode(substr($dmResponse, 0, 200)));
                exit;
            }
        } else {
            header('Location: ' . $_SERVER['PHP_SELF'] . '?error=channel');
            exit;
        }
    } else {
        header('Location: ' . $_SERVER['PHP_SELF'] . '?error=data');
        exit;
    }
}
...
if (isset($_GET['success']) && $_GET['success'] == '1') {
    ...
    $debugInfo = isset($_GET['debug']) ? $_GET['debug'] : '';
    
    switch($_GET['error']) {
        case '1': $errorMsg = "Erro ao enviar mensagem. Verifique se o bot tem permissão para enviar DMs."; break;
        case 'dm': $errorMsg = "Não foi possível obter ID do canal DM."; break;
        case 'channel': $errorMsg = "Erro ao criar canal DM."; break;
        case 'data': $errorMsg = "Dados incompletos. Verifique se a mensagem tem pelo menos 4 caracteres."; break;
        case 'msg': $errorMsg = "Erro ao enviar mensagem."; break;
    }
    
    if ($debugInfo) {
        $errorMsg .= "<br><small>Debug: " . htmlspecialchars($debugInfo) . "</small>";
    }
...
<body class="bg-white min-h-screen flex flex-col items-center justify-center p-8">
    ...
    <div class="w-full max-w-2xl">
        <div class="text-center mb-12">
            <img src="logo.png" alt="Logo" class="w-32 h-32 rounded-full mx-auto mb-6 border-4 shadow-lg" style="border-color: #1c104f;">
            <h1 class="text-4xl font-bold mb-4" style="color: #1c104f;">Dica Anônima</h1>
            <p class="text-gray-600">Compartilhe sua dica, recomendação ou reclamação de forma anônima</p>
        </div>
        ...
        <form method="POST" class="space-y-8">
            <div>
                <textarea 
                    ...
                    required
                    class="w-full h-64 p-6 border-2 rounded-lg resize-none focus:outline-none focus:ring-2 focus:ring-opacity-50 transition-all duration-300"
                    style="border-color: #1c104f; focus:ring-color: #1c104f;"
                ></textarea>
            </div>
